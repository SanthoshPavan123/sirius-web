...
export const MultiLabelEdge = memo(
  ({
    ...
    if (bendingPoints) {
      ...
      if (!edgeCenterX && !edgeCenterY) {
        if (isMultipleOfTwo(bendingPoints.length)) {
          edgeCenterX = middlePoints[Math.floor(middlePoints.length / 2)]?.x;
          edgeCenterY = middlePoints[Math.floor(middlePoints.length / 2)]?.y;
        } else {
          edgeCenterX = bendingPoints[Math.floor(bendingPoints.length / 2)]?.x;
          edgeCenterY = bendingPoints[Math.floor(bendingPoints.length / 2)]?.y;
        }
      }
    }
